...
Enhanced Influence Model is an efficient algorithm to identify a significantly mutated subnetwork with respect to gene interactions. (Vandin, Upfal, and Raphael, 2011)

+ Let $\sigma_i$ be a score for gene $i$, as measured by $\sigma_i = -2\log(p_i)$ where $p_i$ is the p-value obtained from marginal regression. 
...
+ Next, we remove all edges with weights smaller than a threshold $\delta$ to obtain a subgraph. The connected components remained are candidates for genetic pathways.
...
```{r}
# compute symmetric importance score using Enhanced Influence MWodel
...
Since increasing $\delta$ gives us more conservative criteria, we also consider the following values: $1.5\delta = 0.03, 2\delta = 0.04, 3\delta = 0.06, 4\delta = 0.08$.

For different $\delta$ values, we construct graphs for the major component in each network. In the ideal scenario, we expect that the major component contains and only contains the SNP locations involved in genetic pathways. This is one way that we could use to evaluate our results. 

Intuitively, nodes get a lot bigger as $\delta$ increases. This is because we have fewer nodes in the major component.
